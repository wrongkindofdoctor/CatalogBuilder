...
class GfdlCrawler:
    ...
    It finally returns a list of dict. eg {'project': 'CMIP6', 'path':
      '/uda/CMIP6/CDRMIP/NCC/NorESM2-LM/esm-pi-cdr-pulse/r1i1p1f1/Emon/zg/gn/v20191108/zg_Emon_NorESM2-LM_esm-pi-cdr-pulse_r1i1p1f1_gn_192001-192912.nc',
      'variable': 'zg', 'mip_table': 'Emon', 'model': 'NorESM2-LM', 'experiment_id': 'esm-pi-cdr-pulse',
      'ensemble_member': 'r1i1p1f1', 'grid_label': 'gn', 'temporal subset': '192001-192912', 'institute': 'NCC',
      'version': 'v20191108'}
    """
    def __init__(self):
        print('gfdlCrawler initialized')

    def crawl(self, projectdir: str, dictFilter: dict, dictFilterIgnore, logger):
        """
        ...
        if ("modeling_realm" in dictFilter.keys()) & ("frequency" in dictFilter.keys()):
            pat = re.compile('({}/{}/{}/{})'.format(dictFilter["modeling_realm"], "ts", dictFilter["frequency"],
                                                    dictFilter["chunk_freq"]))
            ...
            if pat is not None:
                ...
                for filename in files:
                    ...
                    filepath = os.path.join(dirpath,
                                            filename)  # 1 AR: Bugfix: this needs to join dirpath and filename to get the
                    ...
                    list_bad_modellabel = ["", "piControl", "land-hist", "piClim-SO2", "abrupt-4xCO2", "hist-piAer",
                                           "hist-piNTCF", "piClim-ghg", "piClim-OC", "hist-GHG", "piClim-BC", "1pctCO2"]
                    list_bad_chunklabel = ['DO_NOT_USE']
                    if "source_id" in dictInfo:
                        if dictInfo["source_id"] in list_bad_modellabel:
                            logger.debug("Found experiment name in model column, skipping this possibly bad DRS filename",
                                         filepath)
                        #   continue
